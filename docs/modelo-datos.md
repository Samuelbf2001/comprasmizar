...
Las migraciones declaran buckets privados separados para soportes operativos y expediente de proveedor. Desde [`202609170002_adjuntos_tipos_de_archivo.sql`](../supabase/migrations/202609170002_adjuntos_tipos_de_archivo.sql) el bucket operativo admite PDF, JPEG/PNG/WebP, XLSX/DOCX/PPTX, XLS y `text/plain` (el MIME con el que se guarda un CSV), con dos topes distintos a propósito: el CHECK de la base sigue en 20 MiB como barrera exterior sobre filas históricas, y la aplicación corta en 10 MB (`MAX_PRIVATE_ATTACHMENT_BYTES`), que es el número que se le promete a quien sube. El expediente de proveedor no se amplió: sigue en PDF/JPEG/PNG/WebP. El tipo nunca se deduce de la extensión ni del `Content-Type` del cliente: sale de husmear los bytes (`lib/infrastructure/attachment-mime.ts`), y `tipo='foto'` exige un MIME de imagen. Los clientes JWT no reciben `INSERT`, `UPDATE` ni `DELETE` directo sobre `adjuntos` o `storage.objects`: el servidor prepara una URL de carga firmada no reutilizable, el navegador envía el multipart exigido por Supabase y el servidor hace `HEAD/info` de tamaño y MIME antes de crear metadata legible. La descarga usa una ruta autenticada que emite una URL firmada de 60 segundos con `no-store` y `no-referrer`; ni la clave del objeto ni la URL firmada aparecen en las vistas de listado.
