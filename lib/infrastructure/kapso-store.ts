...
export function createKapsoAttachmentCopier(databaseUrl = runtimeEnv().DATABASE_URL, downloader: KapsoAttachmentDownloader = defaultKapsoAttachmentDownloader): KapsoAttachmentCopier {
  ...
  const audit = (entidadId: string, evento: string, data: Record<string, unknown>) => sql`insert into auditoria (entidad, entidad_id, evento, origen, usuario_id, fecha, datos_json) values ('requisicion_item', ${entidadId}, ${evento.toUpperCase().replace(/[^A-Z0-9_]/g, "_")}, 'kapso', null, now(), ${JSON.stringify(data)}::jsonb)`;
  const logFailure = async (event: KapsoWebhookEvent, requisitionId: string, itemId: string, reason: string) => {
    ...
    await sql`insert into whatsapp_eventos (direccion, telefono, requisicion_id, tipo, payload_json, estado_entrega, kapso_message_id, fecha) values ('entrada', ${phone}, ${requisitionId}, 'flow', ${JSON.stringify({ evento: "adjunto_fallido", itemId, reason })}::jsonb, 'fallido', ${`${event.eventId}:adjunto:${itemId}`}, now()) on conflict (kapso_message_id) where kapso_message_id is not null do nothing`;
