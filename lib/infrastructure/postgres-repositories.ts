...
export class PostgresPorts implements AuditRepository, ConsecutiveRepository, CatalogRepository {
  ...
  // 202609150005: el nombre solo choca entre empresas (NIT contra NIT) — mismo alcance que el índice
  // parcial `proveedores_razon_social_normalizada_unico_idx`; la identificación choca por (tipo, normalizada).
  async findSupplierDuplicate(value: Pick<CatalogSupplier, "name" | "nit" | "identificationType" | "identification">, exceptId?: string): Promise<string | null> { const type = value.identificationType ?? "NIT", identification = value.identification ? normalizeIdentification(value.identification) || null : null; const rows = await this.sql<{ id: string }[]>`select id from proveedores where (${exceptId ?? null}::uuid is null or id <> ${exceptId ?? null}) and ((${type === "NIT"} and tipo_identificacion = 'NIT' and lower(btrim(razon_social)) = lower(btrim(${value.name}))) or (${value.nit ?? null}::text is not null and nit_normalizado = nullif(regexp_replace(${value.nit ?? null}, '[^0-9A-Za-z]', '', 'g'), '')) or (${identification}::text is not null and tipo_identificacion = ${type} and identificacion_normalizada = ${identification})) limit 1`; return rows[0]?.id ?? null; }
