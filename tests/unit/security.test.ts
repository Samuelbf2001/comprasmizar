...
describe("security boundaries", () => {
  ...
  it("audits MCP reads with MCP origin before executing and keeps gated tools out of runtime catalog", async () => { const events: unknown[] = []; const value = await auditMcpTool({ append: async (entry) => { events.push(entry); }, list: async () => [] }, { id: "reader", roles: ["revisor"] }, "consultar_requisiciones", async () => "ok", "33333333-3333-4333-8333-333333333333"); expect(value).toBe("ok"); expect(events).toMatchObject([{ entity: "mcp", entityId: "33333333-3333-4333-8333-333333333333", actorId: "reader", origin: "mcp", data: { tool: "consultar_requisiciones" } }]); expect(MCP_TOOL_NAMES).toEqual(["consultar_requisiciones", "consultar_ordenes", "consultar_gastos", "estado_embudo", "registrar_caja_menor", "actualizar_estado_orden", "ficha_proveedor", "exportar_reporte"]); expect(MCP_GATED_TOOL_NAMES).toContain("crear_requisicion"); expect(MCP_TOOL_NAMES).not.toContain("crear_requisicion"); });
