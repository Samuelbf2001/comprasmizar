...
function fakeReportDeps(expenses: Expense[]): ServiceDependencies {
  ...
  return {
    ...
    // RF-707: el reporte de gastos ahora cruza cada gasto con su orden (pagado/estado de pago).
    orders: { save: notUsed, list: notUsed, listVisibleTo: async () => [], listByRequisition: notUsed, get: notUsed, listAttentionCandidates: notUsed, listRecentlyUpdated: notUsed, dashboardPendingCount: notUsed },
    ...
    orderPayments: { save: notUsed, listByOrder: notUsed, get: notUsed, annul: notUsed, listCash: notUsed },
    pettyCash: { save: notUsed, list: notUsed },
    incomes: { save: notUsed, list: notUsed },
    cashCloses: { get: notUsed, listByCashBox: notUsed, sumMovements: notUsed, previousClosingBalance: notUsed, upsert: notUsed, tagMovements: notUsed, setStatus: notUsed, listMovementsByCostCenter: notUsed },
    ...
    catalogs: { create: notUsed, get: notUsed, update: notUsed, findSupplierDuplicate: notUsed, findSupplierByIdentification: notUsed, findRequesterDuplicate: notUsed, isEligibleApprover: notUsed, hasRequisitionsForWork: notUsed },
...
describe("security boundaries", () => {
  ...
  it("audits MCP reads with MCP origin before executing and keeps gated tools out of runtime catalog", async () => { const events: unknown[] = []; const value = await auditMcpTool({ append: async (entry) => { events.push(entry); }, list: async () => [] }, { id: "reader", roles: ["revisor"] }, "consultar_requisiciones", async () => "ok", "33333333-3333-4333-8333-333333333333"); expect(value).toBe("ok"); expect(events).toMatchObject([{ entity: "mcp", entityId: "33333333-3333-4333-8333-333333333333", actorId: "reader", origin: "mcp", data: { tool: "consultar_requisiciones" } }]); expect(MCP_TOOL_NAMES).toEqual(["consultar_requisiciones", "consultar_ordenes", "consultar_gastos", "estado_embudo", "registrar_pago", "actualizar_estado_orden", "listar_centros_costo", "crear_centro_costo", "ficha_proveedor", "exportar_reporte"]); expect(MCP_GATED_TOOL_NAMES).toContain("crear_requisicion"); expect(MCP_TOOL_NAMES).not.toContain("crear_requisicion"); });
