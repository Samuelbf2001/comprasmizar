...
function deps(options: { feature?: boolean; transactionFeature?: boolean; eligibleApprover?: boolean; failAudit?: boolean; uniqueViolation?: boolean; authUserExists?: boolean; triggerViolation?: "estado" | "rol"; worksWithRequisitions?: Set<string> } = {}) {
  ...
  const catalog = {
    ...
    // N4 (202609150005): el nombre solo choca entre empresas (NIT contra NIT); el NIT y la identificación, siempre.
    findSupplierDuplicate: async (value: { name: string; nit?: string | null; identificationType?: string; identification?: string | null }, exceptId?: string) => [...records.entries()].find(([key, entry]) => { if (!key.startsWith("suppliers:") || entry.id === exceptId) return false; const record = entry as CatalogSupplier; return ((value.identificationType ?? "NIT") === "NIT" && (record.identificationType ?? "NIT") === "NIT" && record.name.toLowerCase() === value.name.toLowerCase()) || (Boolean(value.nit) && record.nit === value.nit) || (Boolean(value.identification) && (record.identificationType ?? "NIT") === (value.identificationType ?? "NIT") && record.identification === value.identification); })?.[1].id ?? null,
  ...
  const transaction = async <T>(_lock: string | undefined, work: (repositories: Parameters<ServiceDependencies["transactions"]["transaction"]>[1] extends (repositories: infer R) => Promise<unknown> ? R : never) => Promise<T>) => { const snapshot = structuredClone([...records.entries()]), auditSnapshot = structuredClone(audits); try { return await work({ catalogs: catalog, audit, requisitions: {} as never, orders: {} as never, expenses: {} as never, orderPayments: {} as never, pettyCash: {} as never, incomes: {} as never, cashCloses: {} as never, consecutives: {} as never, features: { isEnabled: async () => options.transactionFeature ?? options.feature ?? false }, items: {} as never, notifications: {} as never }); } catch (error) { records.clear(); for (const [key, value] of snapshot) records.set(key, value); audits.splice(0, audits.length, ...auditSnapshot); throw error; } };
...
describe("CatalogService", () => {
  ...
  it("catálogo de proveedores: dos personas homónimas con cédulas distintas conviven; dos NIT con la misma razón social o la misma cédula siguen chocando", async () => {
    const fixture = deps();
    await fixture.service.create("suppliers", { name: "Juan Pérez", identificationType: "CC", identification: "71.111.111", active: true }, reviewer);
    await expect(fixture.service.create("suppliers", { name: "Juan Pérez", identificationType: "CC", identification: "71.222.222", active: true }, reviewer)).resolves.toMatchObject({ name: "Juan Pérez" });
    await expect(fixture.service.create("suppliers", { name: "Juan Pérez", identificationType: "NIT", identification: "900.444.444-1", active: true }, reviewer)).resolves.toMatchObject({ identificationType: "NIT" });
    await expect(fixture.service.create("suppliers", { name: "juan pérez", identificationType: "NIT", identification: "900.555.555-1", active: true }, reviewer)).rejects.toMatchObject({ code: "CONFLICT" });
    await expect(fixture.service.create("suppliers", { name: "Otro Nombre", identificationType: "CC", identification: "71.111.111", active: true }, reviewer)).rejects.toMatchObject({ code: "CONFLICT" });
  });
