  ...
  return <MobileFrame><StepIntro code="obra autorizada" onChangeAccess={() => setAccessGranted(false)} /><Progress step={step} />
    <form className={styles.stepCard} onSubmit={submit} noValidate>
      {step === 1 ? <><div className={styles.stepHeader}><p className={styles.stepEyebrow}><ClipboardList aria-hidden="true" size={16} /> Paso 1 de 2</p><h2>¿Para quién y cuándo?</h2><p>Indica el tipo de solicitud, la fecha y tu nombre.</p></div><div className={styles.stepBody}>
        ...
        {!access.workId && <label className={styles.field}><span className={styles.fieldLabel}>Obra <em className={styles.required}>*</em></span><select className={styles.control} name="work" value={values.work} onChange={event => update('work', event.target.value)} disabled={!worksLoaded || publicWorks.length === 0} aria-invalid={Boolean(errors.work)} aria-describedby={errors.work ? 'mobile-production-work-error' : undefined}><option value="" disabled>{worksLoaded ? (publicWorks.length ? 'Selecciona una obra' : 'No hay obras habilitadas') : 'Cargando obras…'}</option>{publicWorks.map(work => <option key={work.id} value={work.id}>{work.name}</option>)}</select>{errors.work && <small className={styles.error} id="mobile-production-work-error">{errors.work}</small>}</label>}
