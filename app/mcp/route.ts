...
function createServer(actor: Awaited<ReturnType<typeof lookupMcpActor>>) {
  ...
  server.registerTool("registrar_caja_menor", { description: "Registra caja menor usando el mismo servicio de la web.", inputSchema: { workId: z.string().uuid(), date: z.string().date(), concept: z.string().min(1), tagId: z.string().uuid(), amount: z.number().int().positive(), cashBoxId: z.string().uuid(), paymentMethod: z.enum(["efectivo", "transferencia", "cheque", "tarjeta", "otro"]), costCenterId: z.string().uuid().optional(), iva: z.number().int().nonnegative().optional() } }, async (input) => result(await audited(actor, "registrar_caja_menor", () => service.registerPettyCash(input, { actor, origin: "mcp" }))));
