...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return authenticatedJson(async (actor) => {
    ...
    switch (input.action) {
      ...
      case "review": return service.review(id, { tagId: input.tagId, approverId: input.approverId, workId: input.workId, costCenterId: input.costCenterId, billedCompanyId: input.billedCompanyId, paymentTerms: input.paymentTerms, items: input.items }, requestContext);
      case "send_for_approval": return service.sendForApproval(id, requestContext);
      case "send_and_approve": return service.sendAndApproveAsMaster(id, requestContext);
