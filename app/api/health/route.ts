...
export function GET() { const core = isRuntimeConfigured(), origin = Boolean(appOrigin()); return Response.json({ status: core && origin ? "ok" : "unconfigured", components: { public: isPublicConfigured(), kapso: isKapsoConfigured(), mcp: isMcpConfigured(), origin }, commit: process.env.APP_COMMIT?.trim() || null }, { status: core && origin ? 200 : 503, headers: { "Cache-Control": "no-store" } }); }
