 ...
 *  RF-601: `identificationType` + `identification` (NIT o cédula) conviven con `nit` (camino legado,
 *  equivale a NIT); el servicio los deja coherentes (SupplierService.resolveSupplierIdentity). */
export const supplierCreateSchema = z.object({ name, nit: optionalNit, identificationType: z.enum(SUPPLIER_IDENTIFICATION_TYPE_VALUES).optional(), identification: optionalNit, pendingNormalization: z.boolean().optional(), contact: contact.optional(), bankDetails: bankDetails.optional(), active: z.boolean().optional() }).strict();
